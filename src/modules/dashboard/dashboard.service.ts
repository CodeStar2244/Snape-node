...
export class DashboardService {
  public getSummary = async (userDetails) => {
    try {
      ...
      const clients = await bookingRepo
        ...
        .getCount();
      const photoGraphy = await bookingRepo
        .createQueryBuilder("bookings")
        .where({ agentid: userDetails.id })
        .andWhere(`"bookings"."enddatetime" >= '${customDate}'`)
        .andWhere(`"bookings"."enddatetime" <= '${todayDate}'`)
        ...
        .andWhere({ bookingstatusid: 10 })
        .getCount();
      const videoGraphy = await bookingRepo
        .createQueryBuilder("bookings")
        .where({ agentid: userDetails.id })
        .andWhere(`"bookings"."enddatetime" >= '${customDate}'`)
        .andWhere(`"bookings"."enddatetime" <= '${todayDate}'`)
        ...
        .andWhere({ bookingstatusid: 10 })
        .getCount();
      const revenue = await bookingRepo
        .createQueryBuilder("bookings")
      ...
      return ResponseBuilder.data({
        ...
        photoGraphy,
        videoGraphy,
        revenue: revenue?.totalamount ? revenue.totalamount : 0,
      });
    } catch (error) {
      console.log(error);
    }
  ...
  public recentCustomers = async (userDetails) => {
    try {
      ...
      const todayDate = moment().endOf("day").format(TIME_STAMP_FORMAT);
      const bookingRepo = AppDataSource.getRepository(Tblbooking);
      const recentCustomers = await bookingRepo
        .createQueryBuilder("bookings")
        .innerJoinAndSelect("bookings.clientid", "clients")
        .innerJoin(
          "tblimages",
          "images",
          "bookings.clientid = images.entityid AND entitytype = 'client'",
        )
