...
import {
  AssetCreateModel,
  AssetUpdateModel,
} from "./enterpriseAssetRegistry.model";
export class EnterpriseAssetRegistryService {
  ...
  public assetDashboard = async (userDetails) => {
    try {
      ...
      const assetsCount = await assetRepo
        .createQueryBuilder("assets")
        .select('count("assets"."id")', "devices")
        .where('"assets"."clientId" = :clientId', { clientId: userDetails.id })
        .getRawOne();
      const totalDevices = +assetsCount.devices;
      const assetGroupByStatusQuery = assetRepo
        .createQueryBuilder("assets")
        .select('count("assets"."id")', "devices")
        .addSelect("assets.status", "status")
        .where('"assets"."clientId" = :clientId', { clientId: userDetails.id })
        .addGroupBy("assets.status");
      const assetGroupByTypeQuery = assetRepo
        .createQueryBuilder("assets")
        .select(
          `round(count("assets"."id")::decimal * 100 / ${totalDevices},2)`,
          "devices",
        )
      ...
      const [assetGroupByStatus, assetGroupByType, getAssetAmounts] =
        await Promise.all([
      ...
      const activeObj = assetGroupByStatus.find(
        (obj: any) => obj.status === "Active",
      );
      summaryInfo.active = activeObj?.devices ? activeObj?.devices : "0";
      const saleObj = assetGroupByStatus.find(
        (obj: any) => obj.status === "For Sale",
      );
      summaryInfo.sale = saleObj?.devices ? saleObj?.devices : "0";
      const lostObj = assetGroupByStatus.find(
        (obj: any) => obj.status === "Lost",
      );
      summaryInfo.lost = lostObj?.devices ? lostObj?.devices : "0";
      const rentObj = assetGroupByStatus.find(
        (obj: any) => obj.status === "For Rent",
      );
      summaryInfo.rent = rentObj?.devices ? rentObj?.devices : "0";

      const cameraObj = assetGroupByType.find(
        (obj: any) => obj.type === "CAMERA",
      );
      categoryInfo.camera = cameraObj?.devices ? cameraObj?.devices : "0";
      const screenObj = assetGroupByType.find(
        (obj: any) => obj.type === "SCREEN",
      );
      categoryInfo.screen = screenObj?.devices ? screenObj?.devices : "0";
      const cellPhoneObj = assetGroupByType.find(
        (obj: any) => obj.type === "CELL_PHONE",
      );
      categoryInfo.cell_phone = cellPhoneObj?.devices
        ? cellPhoneObj?.devices
        : "0";
      const printerObj = assetGroupByType.find(
        (obj: any) => obj.type === "PRINTER",
      );
  ...
  public updateAsset = async (userDetails, params, body) => {
    try {
      ...
      const { nickName, deviceAmount, deviceID, status, type } =
        new AssetUpdateModel(body);
      const updateObject = {
        nickName,
        deviceAmount,
        deviceID,
