...
export class CollectionService {
  ...
  public createCollection = async (body, userDetails) => {
    try {
      const collectionRepository = AppDataSource.getRepository(
        EnterpriseCollections,
      );
      const designRepo = AppDataSource.getRepository(
  ...
  public getCollections = async (userDetails, search, order, sort) => {
    try {
      ...
      const query = await collectionRepository
  ...
  public deleteCollection = async (userDetails, id) => {
    try {
      ...
      const clientSpace =
        await this.enterpriseClientService.getEnterpriseRemaningBalance(
          userDetails,
        );
      return clientSpace;
    } catch (error) {
  ...
  public updateCollection = async (params, body, userDetails) => {
    try {
      ...
      const collection = await collectioRepo.findOneBy({
        id: params.id,
        createdBy: userDetails.id,
      });
      if (!collection) {
      ...
      const {
        ...
        coverPhoto,
        slug,
      } = new UpdateCollectionModel(body);
      ...
      await collectioRepo.save({
        ...collection,
        ...updateObject,
        tags: collectionTagsArr,
      });
  ...
  public collectionDesign = async (params, body, userDetails) => {
    try {
      ...
      const { theme, x, y, gridSpacing, gridStyle, typography } =
        new CollectionDesignModel(body);
      const updateObject = {
        theme,
        focusX: x,
        focusY: y,
  ...
  public changeCoverPhoto = async (params, body, userDetails) => {
    try {
      ...
      const updateCollection = await collectioRepo.update(params.id, {
        coverPhoto: body.url,
      });
  ...
  public uploadFiles = async (params, body, userDetails) => {
    try {
      ...
      const collectionFiles = await this.getCollectionFilesName(
        userDetails,
        collection.id,
      );
      ...
      if (collection.photos === 0) {
        const compressedCollectoinPhoto = await this.utils.compressImage(
          files[0].key,
          params.id,
        );
        collectioRepo.save({
      ...
      for (const file of files) {
        ...
        const compressedKey = await this.utils.compressImage(
          file.key,
          params.id,
        );
        filesUploadArr.push(
