...
export class StudioManagementService {
    public createClient = async (userDetails, body) => {
        try {
            const studioClientRepository =
                AppDataSource.getRepository(StudioClient);
    ...
    public editInvoice = async (params, body, user): Promise<any> => {
        try {
            ...
            const formattedDate = moment(invoice?.paymentDue).format(
                "MMMM D, YYYY"
            );
    ...
    public getInvoiceSummary = async (user) => {
        try {
            const studioInvoiceRepo =
                AppDataSource.getRepository(StudioInvoice);
