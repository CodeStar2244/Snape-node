...
export class ClientService {
    ...
    try {
      ...
      const collection = await collectionRepository
        ...
        .leftJoin(
          "collection_tag_join",
          "tagsJoin",
          "tagsJoin.collectionsId=collections.id",
        )
        .leftJoin(
          "collection_tags",
          "tags",
          "tagsJoin.collectionTagsId=tags.id",
        )
        .leftJoin(
          "collection_design",
          "collectionDesign",
          "collectionDesign.collectionId=collections.id",
        )
        .leftJoin(
          "collection_themes",
          "themes",
          "collectionDesign.theme=themes.id",
        )
      ...
      if (!collection) {
        return ResponseBuilder.badRequest(
          "Collection Not Found or collection not published",
          404,
        );
      }
      const filesCollection = await filesRepository
        ...
        .where("files.collectionId = :collectionId", {
          collectionId: collection.id,
        })
        .getRawMany();
      const passwordCheckCollection = await collectionRepository.findOneBy({
        id: collection.id,
      });

      if (passwordCheckCollection.password) {
        return this.collectionPasswordRequired(
          collection,
          passwordCheckCollection,
          password,
          filesCollection,
        );
      }
  ...
  private collectionPasswordRequired = async (
    collection,
    passwordCheckCollection,
    password,
    filesCollection,
  ...
  public getCollectionDesign = async (userDetails, id) => {
    try {
      ...
      const collection = await collectionRepository.findOneBy({
        id: id,
  ...
  public downloadFile = async (userDetails, id, { pin }, res) => {
    try {
      ...
      if (!collection.download) {
        return ResponseBuilder.badRequest(
          "Downlaod Not allowed for these collection",
        );
      }
      if (collection.downloadPin) {
        return this.collectionFileDownloadPinRequired(
          collection,
          pin,
          file,
          res,
  ...
  private collectionDownloadPinRequired = async (
    collection,
    pin,
    files,
    res,
  ...
  private collectionFileDownloadPinRequired = async (
    collection,
    pin,
    file,
    res,
  ...
  public getClientQuestionnaries = async (id) => {
    try {
  ...
  public submitClientQuestionnaries = async (id, params) => {
    try {
      ...
      const questionnaries = await quesRepo.findOne({
        where: { id },
        relations: ["createdBy"],
      });
      const userDetails = await agentRepo.findOne({
        where: { id: questionnaries?.createdBy?.id },
      });
      ...
      return ResponseBuilder.data({
        data: {},
        message: "Questionnaries submitted successfully",
      });
    } catch (error) {
      console.log(error);
      return ResponseBuilder.badRequest(error?.message);
