...
export class EnterpriseAgentsService {
  public async getAgents(query: AgentGetList, userDetails) {
    try {
      ...
      const favouriteAgents = await enterpriseAgentFavouriteRepo
      ...
      const agentQuery = agentRepo
        ...
        .addSelect(
          `calculate_distance(agent.latitude , agent.longitude,${queryObj.latitude} ,${queryObj.longitude},'K') as distance`,
        )
        .where("agent.isonline = :isonline", { isonline: true })
        .andWhere(
          `calculate_distance(agent.latitude , agent.longitude,${queryObj.latitude} ,${queryObj.longitude},'K') < ${queryObj.range}`,
        );
      if (query.speciality) {
  ...
  public async getFavouriteAgentList(query, userDetails) {
    try {
      ...
      const favouriteAgentsQuery = enterpriseAgentFavouriteRepo
        .createQueryBuilder("fav")
  ...
  public async bookAgentRequest(body: BookAgent, params, userDetails) {
    try {
      ...
      const agentRepo = AppDataSource.getRepository(Tblagent);
      const enterpriseBookingRepo =
        AppDataSource.getRepository(EnterpriseBooking);
  ...
  public async getAgentCategories(params, userDetails) {
    try {
      const { agentId } = params;
      const agentCategoriesRepo = AppDataSource.getRepository(
        Tblagentmediacategoriesmapping,
      );
      const mediaCategoriesRepo =
        AppDataSource.getRepository(Tblmediacategories);
      const agentQuery = agentCategoriesRepo
        .createQueryBuilder("mediamapping")
        .leftJoin(
          "tblmediacategories",
          "categories",
          "categories.id = mediamapping.mediacategoryid",
        )
        .leftJoin(
          "tblimages",
          "images",
          "categories.id = images.entityid AND entitytype = 'mediacategory'",
        )
    ...
    try {
      ...
      const bookings = await bookingRepo
        ...
        .leftJoin(
          "tblimages",
          "images",
          "b.clientId = images.entityid AND entitytype = 'client'",
        )
  ...
  public async addRemoveFavourite(
    params: AgentFavourite,
    userDetails,
    agentId,
  ) {
    try {
      ...
      const agent = await agentRepo.findOne({ where: { id: agentId } });
      const enterpriseClient = await enterpriseRepo.findOne({
        where: { id: userDetails.id },
      });
      const enterpriseAgentFavourite =
        await enterpriseAgentFavouriteRepo.findOne({
          where: {
            clientId: {
      ...
      if (params.isFavourite === "0") {
        ...
        if (enterpriseAgentFavourite) {
          ...
          const enterpriseAgentFavouriteEntry =
            enterpriseAgentFavouriteRepo.create({
              clientId: enterpriseClient,
              agentId: agent,
            });
